...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/options/leaps-table.tsx"
      to: "src/lib/options.ts"
      via: "useMemo calling computeLeapsDisplay with symbolPrices[opt.ticker]"
      pattern: "computeLeapsDisplay.*symbolPrices\\[opt\\.ticker\\]"
---
...
Implementation:
1. Compute display data with useMemo, explicitly wiring symbol prices to each LEAPS option:

```typescript
const leapsRows = useMemo(() => {
  const leapsOptions = options.filter(opt => opt.strategy_type === "LEAPS")
...
This is the critical wiring -- `symbolPrices[opt.ticker]` passes the current market price for each option's underlying ticker into `computeLeapsDisplay`, which uses it to compute intrinsicValue, extrinsicValue, and currentPnl. If the ticker has no price data (e.g. Tiingo doesn't cover it), null is passed and those derived fields show "N/A".

2. Simplification: Since LEAPS typically has ~35 positions and roll chains are less common in LEAPS, use a flat table with LeapsDisplayRow[] and leapsColumns. Skip roll chain sub-rows for LEAPS to reduce complexity. If roll chains are present, they show as individual rows with "Rolled" status badge.
