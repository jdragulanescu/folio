...
must_haves:
  ...
  artifacts:
    ...
    - path: "dashboard/src/components/layout/app-sidebar.tsx"
      provides: "Sidebar with 6 navigation items and active state highlighting"
      contains: "navItems"
    - path: "dashboard/src/components/layout/theme-provider.tsx"
    ...
    - path: "dashboard/src/app/transactions/page.tsx"
      provides: "Transactions placeholder page"
    - path: "dashboard/src/app/options/page.tsx"
      provides: "Options placeholder page"
    - path: "dashboard/src/app/dividends/page.tsx"
      provides: "Dividends placeholder page"
    - path: "dashboard/src/app/deposits/page.tsx"
      provides: "Deposits placeholder page"
    - path: "dashboard/src/app/performance/page.tsx"
      provides: "Performance placeholder page"
  key_links:
    - from: "dashboard/src/app/layout.tsx"
      to: "dashboard/src/components/layout/theme-provider.tsx"
      via: "import ThemeProvider"
      pattern: "import.*ThemeProvider"
    - from: "dashboard/src/app/layout.tsx"
      to: "dashboard/src/components/layout/app-sidebar.tsx"
      via: "import AppSidebar"
      pattern: "import.*AppSidebar"
    - from: "dashboard/src/components/layout/app-sidebar.tsx"
      to: "next/link"
...
<task type="auto">
  ...
  <action>
    ...
    4. Create `dashboard/.env.example` with these placeholder variables (NO actual secrets):
    ```
    # NocoDB Connection
    ...
    5. Add `.env.local` to `dashboard/.gitignore` if not already there (create-next-app typically includes this).
  ...
  <verify>
    - `ls dashboard/package.json` confirms the project exists
    - `cd dashboard && pnpm build` compiles without errors
    - `grep "next-themes" dashboard/package.json` shows the dependency
    - `grep "server-only" dashboard/package.json` shows the dependency
    - `ls dashboard/src/components/ui/sidebar.tsx` confirms shadcn sidebar installed
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **theme-provider.tsx:** Create `dashboard/src/components/layout/theme-provider.tsx`:
    ...
    **app-sidebar.tsx:** Create `dashboard/src/components/layout/app-sidebar.tsx`:
    - "use client" directive at top
  ...
  <verify>
    ...
    - `grep "@custom-variant dark" dashboard/src/styles/globals.css` confirms dark mode variant
    - `grep "defaultTheme.*dark" dashboard/src/components/layout/theme-provider.tsx` confirms dark default
    - `grep "SidebarProvider" dashboard/src/app/layout.tsx` confirms sidebar wiring
    - `grep "SidebarTrigger" dashboard/src/app/layout.tsx` confirms mobile trigger
    - `ls dashboard/src/app/transactions/page.tsx dashboard/src/app/options/page.tsx dashboard/src/app/dividends/page.tsx dashboard/src/app/deposits/page.tsx dashboard/src/app/performance/page.tsx` confirms all placeholder pages exist
  </verify>
  <done>
