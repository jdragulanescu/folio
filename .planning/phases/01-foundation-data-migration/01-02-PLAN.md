...
must_haves:
  ...
  artifacts:
    - path: "dashboard/src/lib/nocodb.ts"
    ...
    - path: "dashboard/src/lib/types.ts"
      ...
      contains: "BROKERS"
  key_links:
    - from: "dashboard/src/lib/nocodb.ts"
      to: "server-only"
...
<task type="auto">
  ...
  <action>
    Create `dashboard/src/lib/types.ts` with interfaces matching the NocoDB table schemas from FOLIO-IMPLEMENTATION-PLAN.md.
  ...
  <verify>
    - `cd dashboard && pnpm tsc --noEmit` — no type errors
    - `grep "BROKERS" dashboard/src/lib/types.ts` — broker constants exist
    - `grep "SymbolRecord" dashboard/src/lib/types.ts` — all 8 record types present
    - `grep "NocoDBListResponse" dashboard/src/lib/types.ts` — pagination type present
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>dashboard/src/lib/nocodb.ts</files>
  <action>
    Create `dashboard/src/lib/nocodb.ts` with:
  ...
  <verify>
    - `cd dashboard && pnpm tsc --noEmit` — no type errors
    - `grep 'import "server-only"' dashboard/src/lib/nocodb.ts` — server-only guard present as first import
    - `grep "export async function" dashboard/src/lib/nocodb.ts` — all 6 functions exported
    - `grep "NEXT_PUBLIC" dashboard/src/lib/nocodb.ts` — must return NO results (env vars are server-only)
    - `grep "cache.*no-store" dashboard/src/lib/nocodb.ts` — no-store caching confirmed
    - `cd dashboard && pnpm build` — build succeeds
  </verify>
  <done>
...
<verification>
1. `cd dashboard && pnpm build` — build succeeds without errors
2. `cd dashboard && pnpm tsc --noEmit` — TypeScript compilation clean
3. `grep 'import "server-only"' dashboard/src/lib/nocodb.ts` returns a match
4. `grep "NEXT_PUBLIC" dashboard/src/lib/nocodb.ts` returns zero matches
5. `dashboard/src/lib/types.ts` exports interfaces for all 8 tables plus broker constants
6. `dashboard/src/lib/nocodb.ts` exports listRecords, getAllRecords, getRecord, createRecord, updateRecord, fetchParallel
